...
#arr_f = np.loadtxt("fpn_res_rand_outputs_10_48640_60000_11.csv",delimiter=",",dtype=int)
arr_tm = np.loadtxt("mask_256.csv", delimiter=",").astype(np.int64)
arr_f=np.loadtxt("class_faulty_0_new.csv", delimiter=",",dtype=int)
#arr_f=arr_f[:,:-1]
...
for max_class in range(0, 19):
    ...
    for i in range(faulty_img.shape[0]):
        for j in range(faulty_img.shape[1]):
            if truth_mask_img[i, j] == 1 and faulty_img[i, j] == 1:
                tp += 1
            elif truth_mask_img[i, j] == 0 and faulty_img[i, j] == 0:
                tn += 1   
            elif truth_mask_img[i, j] > faulty_img[i, j]:
                fp += 1
            elif truth_mask_img[i, j] < faulty_img[i, j]:
                fn += 1
            '''
