...
unsigned int num_glitches=50;
unsigned int width;
unsigned int step=150;
unsigned int glitch_step=2;
unsigned int limit;
	      ...
	      axi_vptr[ii]=offset+num_glitches*glitch_step;
                    ...
                    for (unsigned int j = 0; j < 2; j++) {
                        ...
                        if(j % 2 == 1) {
                              ...
                              if (axi_vptr[199] - (min_offset + k * step) > 2000 && axi_vptr[199] - (min_offset + k * step) <= 5000){
                              printf("%u,%d,%u\n",offset+k*step,diff_pixel,axi_vptr[199]);
                              //printf("Diff pixels:%u,%u,%u\n",colorx[0],colorx[1],colorx[2]);
                              std::string filename = std::string("test-end-faulty-mob")  +  "-"+ std::to_string(axi_vptr[199] - (min_offset + k * step))  +"-" + std::to_string(min_offset + k * step) + "-" + std::to_string(axi_vptr[199]) +  "-" +std::to_string(diff_pixel) + ".png";
